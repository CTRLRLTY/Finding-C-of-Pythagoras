...
def pool_handler(n: int,min_range: int = 0,batch_size: int = 1000000) -> None:
    total = min_range - min_range % 10 # set last digit to 0
        ...
        while not mpmath.isint(a): #if i=1 is not the answer
            ...
            for result in pool.imap_unordered(get_a, params):
                if isinstance(result,tuple):
                    a = result[-3] # Setting the a first so the parent while loop breaks as well
                    ctr = 0
                    for key,WIN in RESULT_WINS:
